...
fn f_int(aa: &Float, y1in: &Complex, y2in: &Complex, x0in: &Complex) -> Complex {
    ...
    let y1: Complex = if (y1in.imag().clone()).abs() < CHOP_TOL {
        Complex::with_val(PREC,y1in.real().clone())
    } else {y1in.clone()};

    let y2: Complex = if (y2in.imag().clone()).abs() < CHOP_TOL {
        Complex::with_val(PREC,y2in.real().clone())
    } else {y2in.clone()};

    let x0: Complex = if (x0in.imag().clone()).abs() < CHOP_TOL {
        Complex::with_val(PREC,x0in.real().clone())
    } else {x0in.clone()};
    ...
    if &0 < rex0 && rex0 < &1 && imx0.clone().abs() < CHOP_TOL {
		let derivcritx0: Complex = (y1.clone() - &y2)/2/(rex0 - y2.clone())/(-(rex0-y1.clone())).pow(2u8).sqrt();
		let signx0: f64;
...
fn prefactor(a: &Float, y1: &Complex, y2: &Complex) -> Complex {
    // We can simplify this function a lot more, just return +/- 1/sqrt(a) depending on the quadrants of y1 and y2
    let y2re = y2.real();
    let y1re = y1.real();
    let y2im = y2.imag();
    let y1im = y1.imag();
    ...
    if y2im.clone().abs() < CHOP_TOL && y1im.clone().abs() < CHOP_TOL {
        ...
        // if y1re.clone().abs() < CHOP_TOL && y2re.clone().abs() < CHOP_TOL
			return Complex::with_val(PREC,1/a.clone()).sqrt()
        }

	} else if y2im.clone().abs() >= CHOP_TOL && y1im.clone().abs() < CHOP_TOL {
		if y1re.clone().abs() >= CHOP_TOL { 
			return Complex::with_val(PREC,-y1re.clone()).sqrt() * (-y2.clone()).sqrt()/(y2.clone() * a * y1re).sqrt()
        }
		else {
		...
		if y2re.clone().abs() >= CHOP_TOL { 
			return (-y1.clone()).sqrt()*Complex::with_val(PREC,-y2re.clone()).sqrt()/(a*y1.clone()*y2re).sqrt()
        }
		else {
